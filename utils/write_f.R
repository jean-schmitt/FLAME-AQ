...
#' write_complete_multi_factor_sens_a_f
#' Run the function "function_tbc" for the scenario and accross all possible combinations of the attribute values specified in "sens_tbc"
#' @param function_tbc Function to simulate (length=1)
#' @param scen_tbc Scenario to consider
#' @param sens_tbc Sensitivity analysis to consider
#' @param get_default_attr_val Shall it consider the default values of the assessed attributes?
#' @export
write_complete_multi_factor_sens_a_f <- function(outputs_path,function_tbc,scen_tbc,sens_tbc,get_default_attr_val="n"){
  #Load input data
  load_input_data_f()
  #Load attribute values
  load_attribute_value()
  ...
  list_depend_fct_scenario <- get_dependent_functions(attribute_list=rownames(scen_attributes))
  #Get the list of attributes to include in the sensitivity analysis
  sens_analysis_attributes <- get_complete_mf_sens_analysis_attributes(outputs_path,sens_tbc)
  list_depend_fct_sens_a <- get_dependent_functions(attribute_list=rownames(sens_analysis_attributes))
  #Output files
  results_l <- list()
  del_fun_res("all")
  #Loop on scenarios
  for (scen_number in seq_len(ncol(scen_attributes))){
    ...
    del_fun_res(list_depend_fct_scenario)
    #Loop on sensitivty analysis attributes
    for (sens_a_number in seq_len(ncol(sens_analysis_attributes))){
      #Update the attribute values with the simulation's values
      update_attribute_values(sens_analysis_attributes[,sens_a_number,drop=FALSE])
      #Clear the functions' results
      del_fun_res(list_depend_fct_sens_a)
      #Get function results
      results <- append(do.call(get(function_tbc), list()),
                        append(setNames(list(sapply(rownames(sens_analysis_attributes),function(x)sens_analysis_attributes[x,sens_a_number])),"Sensitivity"),
                               setNames(list(sapply(rownames(scen_attributes),function(x)scen_attributes[x,scen_number])),"Scenario")))
      #Store results in a new list element
      results_l[[length(results_l) + 1]] <- results
    }
  ...
  #Write results
  save(list="results_l",file=paste0(outputs_path,"/results/mf_complete_sens_a_",function_tbc,"_",sens_tbc,"_",scen_tbc,".RData"))
  reset_attribute_values()
  del_fun_res("all")
}
