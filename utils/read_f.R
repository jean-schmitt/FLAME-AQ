...
read_sens_analysis_f<-function(outputs_path,
  ...
  if (def_results=="y"){
    ...
    dts[,"Sens_attribute_name"] <- "Default"
  } else{
    dts <- NULL
  }
  for (i in 1:length(results_l)){
...
#' read_complete_multi_factor_sens_a_f
#'
#' Function that read the outputs "dts_names" of the function "function_tbc" for a complete multi-factoriel sensitivty analysis with the attributes specified in "sens_tbc"
#' @param function_tbc Function
#' @param sub_function_tbc Subfunction if specified
#' @param dts_name Name of the dataframe
#' @param scen_tbc Scenario to consider
...
#' @param def_results Shall it consider default values?
#' @export
read_complete_multi_factor_sens_a_f<-function(outputs_path,
                                              function_tbc,
                                              sub_function_tbc="n",
                                              dts_name,
                                              sens_tbc,
  ...
  sens_analysis_attribute_list <- get_sens_analysis_attributes(outputs_path,sens_tbc,function_tbc)
  #Read output files
  load(file=paste0(outputs_path,"/results/mf_complete_sens_a_",function_tbc,"_",sens_tbc,"_",scen_tbc,".RData"))
  #Clean the environment
  #Get default table
  if (def_results=="y"){
    results <- do.call(read_scenario_f,list(outputs_path=outputs_path,function_tbc=function_tbc,sub_function_tbc=sub_function_tbc,scen_tbc=scen_tbc))
    dts <- results[[dts_name]]
    for (sens_a_att in sens_analysis_attribute_list){
      dts[,sens_a_att] <- get_default_value(sens_a_att)
    }
  } else{
    dts <- NULL
  }
  for (i in 1:length(results_l)){
    ...
    if (!is.null(temp_dt)){
      temp_dt[,"Scenario"] <- colnames(scen_attributes)[sapply(seq_len(ncol(scen_attributes)),function(x)all(scen_attributes[,x]==results_l[[i]][["Scenario"]][rownames(scen_attributes)]))]
      for (sens_a_att in sens_analysis_attribute_list){
        temp_dt[,sens_a_att] <- results_l[[i]][["Sensitivity"]][[sens_a_att]]
      }
      #Merge results
      dts <- rbind(dts,temp_dt)
    }
