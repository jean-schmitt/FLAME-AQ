...
#' @export
fun_res_f <- function(fun_name,res_env="y"){
  #Environmental setup
  ...
  if (exists(paste0(fun_name,"_res"),where = fun.res.env) & res_env=="y"){
    #Get the results of the function from fun.res.env
    fun_res <- get(paste0(fun_name,"_res"), envir = fun.res.env)
  } else if (!exists(paste0(fun_name,"_res"),where = fun.res.env) & res_env=="y"){
...
fun_res_with_attributes <- function(fun_name,
  ...
  res <- do.call(fun_res_f,list(fun_name=fun_name))
  reset_attribute_values()
  return(res)
}
...
#' get_complete_mf_sens_analysis_attributes
#'
#' Returns the list of attributes and attributes values for all combinations of parameters
#' @param sens_tbc Sensitivity analyses
#' @param function_tbc Function
#' @import readxl
#' @export
get_complete_mf_sens_analysis_attributes <- function(outputs_path,sens_tbc){
  #Input
  sens_analysis <- as.data.frame(read_excel(paste0(outputs_path,"/sens_analysis.xlsx"),sheet = "single_factor",.name_repair = "minimal"))
  #Get the list of attributes to be analysed
  param_tbc <- unlist(strsplit(sens_analysis[sens_analysis$Sensitivity==sens_tbc,"Attributes"],split=";"))
  #Get the list of all values for all attributes
  cont_attr_values <- setNames(lapply(param_tbc,function(x)get_all_attribute_value(x,get_default_attr_val = "y")),param_tbc)
  #Create matrix with all values
  matrix <- matrix(NA, nrow=length(param_tbc),ncol=nrow(expand.grid(cont_attr_values)),dimnames = list(param_tbc,1:nrow(expand.grid(cont_attr_values))))
  #Populate the matrix
  matrix[param_tbc,] <- t(expand.grid(cont_attr_values))[param_tbc,]
  return(matrix)
}
...
#' load_input_data_f
#'
#' Read and save all input data in a new environment
#' @export
load_input_data_f <- function(){
  #Delete data input environment if already existing
