...
vehicle_hist_fc_f = function(vehicle,first_yr=NA,last_yr=NA,fc_ev_mdl=NA,fc_conv_mdl=NA){
  ...
  if (any(is.na(vehicle$fuel_consumption[,colnames(vehicle$fuel_consumption)<first_yr]))){
    min_year <- min(colnames(vehicle$fuel_consumption)[which(!is.na(vehicle$fuel_consumption[1,]))])
    na_cols <- colnames(vehicle$fuel_consumption)[is.na(vehicle$fuel_consumption[1,]) & colnames(vehicle$fuel_consumption)<as.numeric(min_year)]
