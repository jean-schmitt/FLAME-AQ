cobra_health_impact_f <- function(state_resolved_fleet_direct_emissions, fleet_emissions_elec_state, fleet_emissions_elec_county, fleet_fuel_usage_US, last_yr = NA, scenario_id = NA, fleet_electricity_consumption_source = NA, relative_transportation_electricity_demand = NA, calculation_mode = NA) {
  ...
  index_redined_distribution <- which(scenario$TIER1 == 9 & (scenario$TIER2 == 4 | scenario$TIER2 == 5 | scenario$TIER2 == 6))
  ...
  if (calculation_mode == "default" | calculation_mode == "all") {
    for (i in (first_proj_year+1):last_yr) {
      m <- m+1
      for (j in 1:length(states_correspondence$COBRA_code)) {
        state_index <- which(scenario$stid == states_correspondence$COBRA_code[j])
      }
    }
  }
  ...
  if (calculation_mode == "default" | calculation_mode == "all") {
    for (i in (first_proj_yr+1):last_yr) {
      ...
      for (j in 1:length(states_correspondence$COBRA_code)) {
        ...
        #temp <- which(scenario$stid == states_correspondence$COBRA_code[j] & scenario$TIER1 == 11 & scenario$TIER3 == 2)
        #yearly_change_NOx_emissions[m,j] <- fleet_direct_emissions_state_breakdown$NOx[which(fleet_direct_emissions_state_breakdown$Year == i & fleet_direct_emissions_state_breakdown$State == states_correspondence$COBRA_code[j])]/fleet_direct_emissions_state_breakdown$NOx[which(fleet_direct_emissions_state_breakdown$Year == i-1 & fleet_direct_emissions_state_breakdown$State == states_correspondence$COBRA_code[j])]
        #yearly_change_SO2_emissions[m,j] <- fleet_direct_emissions_state_breakdown$SO2[which(fleet_direct_emissions_state_breakdown$Year == i & fleet_direct_emissions_state_breakdown$State == states_correspondence$COBRA_code[j])]/fleet_direct_emissions_state_breakdown$SO2[which(fleet_direct_emissions_state_breakdown$Year == i-1 & fleet_direct_emissions_state_breakdown$State == states_correspondence$COBRA_code[j])]
        #yearly_change_PM25_emissions[m,j] <-  (fleet_direct_emissions_state_breakdown$Total_PM25[which(fleet_direct_emissions_state_breakdown$Year == i & fleet_direct_emissions_state_breakdown$State == states_correspondence$COBRA_code[j])]+fleet_direct_emissions_state_breakdown$Brake_PM25[which(fleet_direct_emissions_state_breakdown$Year == i & fleet_direct_emissions_state_breakdown$State == states_correspondence$COBRA_code[j])]+fleet_direct_emissions_state_breakdown$Tire_PM25[which(fleet_direct_emissions_state_breakdown$Year == i & fleet_direct_emissions_state_breakdown$State == states_correspondence$COBRA_code[j])])/(fleet_direct_emissions_state_breakdown$Total_PM25[which(fleet_direct_emissions_state_breakdown$Year == i-1 & fleet_direct_emissions_state_breakdown$State == states_correspondence$COBRA_code[j])]+fleet_direct_emissions_state_breakdown$Brake_PM25[which(fleet_direct_emissions_state_breakdown$Year == i-1 & fleet_direct_emissions_state_breakdown$State == states_correspondence$COBRA_code[j])]+fleet_direct_emissions_state_breakdown$Tire_PM25[which(fleet_direct_emissions_state_breakdown$Year == i-1 & fleet_direct_emissions_state_breakdown$State == states_correspondence$COBRA_code[j])])
        #scenario$NO2[temp] <- baseline$NO2[temp]*yearly_change_NOx_emissions[m,j]
        #scenario$SO2[temp] <- baseline$SO2[temp]*yearly_change_SO2_emissions[m,j]
        #scenario$PM25[temp] <- baseline$PM25[temp]*yearly_change_PM25_emissions[m,j]
        # Modify the emissions from fuel extraction, processing, transfer, and storage
        # Method 2 - Based on changes in demand distributed over PADDs
        temp_petroleum <- which(scenario$stid == states_correspondence$COBRA_code[j] & scenario$TIER1 == 6 & scenario$TIER2 == 1 & scenario$TIER3 == 99)
        temp_petroleum_storage <- which(scenario$stid == states_correspondence$COBRA_code[j] & scenario$TIER1 == 9 & scenario$TIER2 == 2 & (scenario$TIER3 == 2 | scenario$TIER3 == 4 | scenario$TIER3 == 6 | scenario$TIER3 == 8))
        temp_refining <- which(scenario$stid == states_correspondence$COBRA_code[j] & scenario$TIER1 == 6 & scenario$TIER2 == 2)
        temp_refined_storage <- which(scenario$stid == states_correspondence$COBRA_code[j] & scenario$TIER1 == 9 & scenario$TIER2 == 2 &(scenario$TIER3 == 1 | scenario$TIER3 == 3 | scenario$TIER3 == 5 | scenario$TIER3 == 7 | scenario$TIER3 == 9 | scenario$TIER3 == 10))
        temp_refined_storage_bis <- which(scenario$stid == states_correspondence$COBRA_code[j] & scenario$TIER1 == 9 & scenario$TIER2 == 3 & scenario$TIER3 != 5)
        temp_refined_distribution <- which(scenario$stid == states_correspondence$COBRA_code[j] & scenario$TIER1 == 9 & (scenario$TIER2 == 4 | scenario$TIER2 == 5 | scenario$TIER2 == 6))
        temp_ethanol <- which(scenario$stid == states_correspondence$COBRA_code[j] & scenario$TIER1 == 7 & scenario$TIER2 == 99 & scenario$TIER3 == 1)
        temp_ng <- which(scenario$stid == states_correspondence$COBRA_code[j] & scenario$TIER1 == 6 & scenario$TIER2 == 1 & scenario$TIER3 == 1)
        ...
        temp_petroleum <- c(temp_petroleum, temp_petroleum_storage)
        scenario$NO2[temp_petroleum] <- baseline$NO2[temp_petroleum]*rel_crude
        scenario$SO2[temp_petroleum] <- baseline$SO2[temp_petroleum]*rel_crude
        scenario$NH3[temp_petroleum] <- baseline$NH3[temp_petroleum]*rel_crude
        scenario$PM25[temp_petroleum] <- baseline$PM25[temp_petroleum]*rel_crude
        scenario$VOC[temp_petroleum] <- baseline$VOC[temp_petroleum]*rel_crude
        # Refining and storage of final products is linked to the refining factor
        temp_refining <- c(temp_refining, temp_refined_storage, temp_refined_storage_bis)
        scenario$NO2[temp_refining] <- baseline$NO2[temp_refining]*rel_refining
        scenario$SO2[temp_refining] <- baseline$SO2[temp_refining]*rel_refining
        scenario$NH3[temp_refining] <- baseline$NH3[temp_refining]*rel_refining
        scenario$PM25[temp_refining] <- baseline$PM25[temp_refining]*rel_refining
        scenario$VOC[temp_refining] <- baseline$VOC[temp_refining]*rel_refining
        # Distribution is linked to the demand factor
        scenario$NO2[temp_refined_distribution] <- baseline$NO2[temp_refined_distribution]*rel_demand
        scenario$SO2[temp_refined_distribution] <- baseline$SO2[temp_refined_distribution]*rel_demand
        scenario$NH3[temp_refined_distribution] <- baseline$NH3[temp_refined_distribution]*rel_demand
        scenario$PM25[temp_refined_distribution] <- baseline$PM25[temp_refined_distribution]*rel_demand
        scenario$VOC[temp_refined_distribution] <- baseline$VOC[temp_refined_distribution]*rel_demand
        # Ethanol production
        scenario$NO2[temp_ethanol] <- baseline$NO2[temp_ethanol]*rel_ethanol
        scenario$SO2[temp_ethanol] <- baseline$SO2[temp_ethanol]*rel_ethanol
        scenario$NH3[temp_ethanol] <- baseline$NH3[temp_ethanol]*rel_ethanol
        scenario$PM25[temp_ethanol] <- baseline$PM25[temp_ethanol]*rel_ethanol
        scenario$VOC[temp_ethanol] <- baseline$VOC[temp_ethanol]*rel_ethanol
        # Natural gas emissions
        scenario$NO2[temp_ng] <- baseline$NO2[temp_ng]*rel_ng
        scenario$SO2[temp_ng] <- baseline$SO2[temp_ng]*rel_ng
        scenario$NH3[temp_ng] <- baseline$NH3[temp_ng]*rel_ng
        scenario$PM25[temp_ng] <- baseline$PM25[temp_ng]*rel_ng
        scenario$VOC[temp_ng] <- baseline$VOC[temp_ng]*rel_ng
        # Generate the output file for oil-related emissions
        temp_total_oil <- c(temp_petroleum, temp_refining, temp_refined_distribution, rel_ethanol)
        emissions_oil_related_activities[nrow(emissions_oil_related_activities)+1,] <- c(states_correspondence$COBRA_code[j], i, "tons", sum(scenario$NO2[temp_total_oil]), sum(scenario$SO2[temp_total_oil]), sum(scenario$PM25[temp_total_oil]), sum(scenario$NH3[temp_total_oil]), sum(scenario$VOC[temp_total_oil])) 
        if (fleet_electricity_consumption_source == "FLAME") {
          ...
          for (k in 1:dim(counties)[1]) {
            temp <- which(scenario$sourceindx == counties$sourceindx[k] & scenario$TIER1 == 11 & scenario$TIER3 == 2)
            ...
            if (i == (first_proj_yr+1)) { #In the first year, overwrite the data in the baseline file
              baseline$NO2[temp] <- fleet_emissions_state$NOx[which(fleet_emissions_state$Year == i-1 & fleet_emissions_state$State == states_correspondence$COBRA_code[j])]*county_pop_factor/length(scenario$NO2[temp])
              baseline$SO2[temp] <- fleet_emissions_state$SO2[which(fleet_emissions_state$Year == i-1 & fleet_emissions_state$State == states_correspondence$COBRA_code[j])]*county_pop_factor/length(scenario$NO2[temp])
              baseline$PM25[temp] <- (fleet_emissions_state$Total_PM25[which(fleet_emissions_state$Year == i-1 & fleet_emissions_state$State == states_correspondence$COBRA_code[j])]+fleet_emissions_state$Brake_PM25[which(fleet_emissions_state$Year == i-1 & fleet_emissions_state$State == states_correspondence$COBRA_code[j])]+fleet_emissions_state$Tire_PM25[which(fleet_emissions_state$Year == i-1 & fleet_emissions_state$State == states_correspondence$COBRA_code[j])])*county_pop_factor/length(scenario$NO2[temp])
            }
            ...
            scenario$NO2[temp] <- fleet_emissions_state$NOx[which(fleet_emissions_state$Year == i & fleet_emissions_state$State == states_correspondence$COBRA_code[j])]*county_pop_factor/length(scenario$NO2[temp])
            scenario$SO2[temp] <- fleet_emissions_state$SO2[which(fleet_emissions_state$Year == i & fleet_emissions_state$State == states_correspondence$COBRA_code[j])]*county_pop_factor/length(scenario$NO2[temp])
            scenario$PM25[temp] <- (fleet_emissions_state$Total_PM25[which(fleet_emissions_state$Year == i & fleet_emissions_state$State == states_correspondence$COBRA_code[j])]+fleet_emissions_state$Brake_PM25[which(fleet_emissions_state$Year == i & fleet_emissions_state$State == states_correspondence$COBRA_code[j])]+fleet_emissions_state$Tire_PM25[which(fleet_emissions_state$Year == i & fleet_emissions_state$State == states_correspondence$COBRA_code[j])])*county_pop_factor/length(scenario$NO2[temp])
            # Calculation of the emissions from electricity
            temp_sourceindx <- which(scenario$sourceindx == counties$sourceindx[k] & scenario$TIER1 == 1)
            temp_fips <- which(fleet_emissions_elec_county$FIPS == counties$FIPS[k] & fleet_emissions_elec_county$Year == i)
            scenario$NO2[temp_sourceindx] <- baseline$NO2[temp_sourceindx] + (fleet_emissions_elec_county$NOx[temp_fips]-fleet_emissions_elec_county$NOx[which(fleet_emissions_elec_county$FIPS == counties$FIPS[k] & fleet_emissions_elec_county$Year == i-1)])/length(scenario$NO2[temp_sourceindx])
            scenario$SO2[temp_sourceindx] <- baseline$SO2[temp_sourceindx] + (fleet_emissions_elec_county$SO2[temp_fips]-fleet_emissions_elec_county$SO2[which(fleet_emissions_elec_county$FIPS == counties$FIPS[k] & fleet_emissions_elec_county$Year == i-1)])/length(scenario$SO2[temp_sourceindx])
            scenario$NH3[temp_sourceindx] <- baseline$NH3[temp_sourceindx] + (fleet_emissions_elec_county$NH3[temp_fips]-fleet_emissions_elec_county$NH3[which(fleet_emissions_elec_county$FIPS == counties$FIPS[k] & fleet_emissions_elec_county$Year == i-1)])/length(scenario$NH3[temp_sourceindx])
            scenario$PM25[temp_sourceindx] <- baseline$PM25[temp_sourceindx] + (fleet_emissions_elec_county$PM25[temp_fips]-fleet_emissions_elec_county$PM25[which(fleet_emissions_elec_county$FIPS == counties$FIPS[k] & fleet_emissions_elec_county$Year == i-1)])/length(scenario$PM25[temp_sourceindx])
            scenario$VOC[temp_sourceindx] <- baseline$VOC[temp_sourceindx] + (fleet_emissions_elec_county$VOC[temp_fips]-fleet_emissions_elec_county$VOC[which(fleet_emissions_elec_county$FIPS == counties$FIPS[k] & fleet_emissions_elec_county$Year == i-1)])/length(scenario$VOC[temp_sourceindx])
          }
        }
      }
      # Modify the emissions from fuel extraction, processing, transfer and storage
      # Method 1 - Linear decrease of the emissions
      #temp_petroleum <- which(scenario$TIER1 == 6 & scenario$TIER2 != 3)
      #temp_storage <- which(scenario$TIER1 == 9 & scenario$TIER2 <= 6 & scenario$TIER2 >= 2)
      #scenario$NO2[temp_petroleum] <- baseline$NO2[temp_petroleum]*0.9
      #scenario$NO2[temp_storage] <- baseline$NO2[temp_storage]*0.9
      #scenario$SO2[temp_petroleum] <- baseline$SO2[temp_petroleum]*0.9
      #scenario$SO2[temp_storage] <- baseline$SO2[temp_storage]*0.9
      #scenario$NH3[temp_petroleum] <- baseline$NH3[temp_petroleum]*0.9
      #scenario$NH3[temp_storage] <- baseline$NH3[temp_storage]*0.9
      #scenario$PM25[temp_petroleum] <- baseline$PM25[temp_petroleum]*0.9
      #scenario$PM25[temp_storage] <- baseline$PM25[temp_storage]*0.9
      #scenario$VOC[temp_petroleum] <- baseline$VOC[temp_petroleum]*0.9
      #scenario$VOC[temp_storage] <- baseline$VOC[temp_storage]*0.9
  ...
  if (calculation_mode == "aggregated" | calculation_mode == "all") {
    ...
    for (j in 1:length(states_correspondence$COBRA_code)) {
      # Emissions from the fleet
      #temp <- which(scenario$stid == states_correspondence$COBRA_code[j] & scenario$TIER1 == 11 & scenario$TIER3 == 2)
      #yearly_change_NOx_emissions[m,j] <- fleet_direct_emissions_state_breakdown$NOx[which(fleet_direct_emissions_state_breakdown$Year == last_yr & fleet_direct_emissions_state_breakdown$State == states_correspondence$COBRA_code[j])]/fleet_direct_emissions_state_breakdown$NOx[which(fleet_direct_emissions_state_breakdown$Year == first_proj_yr+1 & fleet_direct_emissions_state_breakdown$State == states_correspondence$COBRA_code[j])]
      #yearly_change_SO2_emissions[m,j] <- fleet_direct_emissions_state_breakdown$SO2[which(fleet_direct_emissions_state_breakdown$Year == last_yr & fleet_direct_emissions_state_breakdown$State == states_correspondence$COBRA_code[j])]/fleet_direct_emissions_state_breakdown$SO2[which(fleet_direct_emissions_state_breakdown$Year == first_proj_yr+1 & fleet_direct_emissions_state_breakdown$State == states_correspondence$COBRA_code[j])]
      #yearly_change_PM25_emissions[m,j] <-  (fleet_direct_emissions_state_breakdown$Total_PM25[which(fleet_direct_emissions_state_breakdown$Year == last_yr & fleet_direct_emissions_state_breakdown$State == states_correspondence$COBRA_code[j])]+fleet_direct_emissions_state_breakdown$Brake_PM25[which(fleet_direct_emissions_state_breakdown$Year == last_yr & fleet_direct_emissions_state_breakdown$State == states_correspondence$COBRA_code[j])]+fleet_direct_emissions_state_breakdown$Tire_PM25[which(fleet_direct_emissions_state_breakdown$Year == last_yr & fleet_direct_emissions_state_breakdown$State == states_correspondence$COBRA_code[j])])/(fleet_direct_emissions_state_breakdown$Total_PM25[which(fleet_direct_emissions_state_breakdown$Year == first_proj_yr+1 & fleet_direct_emissions_state_breakdown$State == states_correspondence$COBRA_code[j])]+fleet_direct_emissions_state_breakdown$Brake_PM25[which(fleet_direct_emissions_state_breakdown$Year == first_proj_yr+1 & fleet_direct_emissions_state_breakdown$State == states_correspondence$COBRA_code[j])]+fleet_direct_emissions_state_breakdown$Tire_PM25[which(fleet_direct_emissions_state_breakdown$Year == first_proj_yr+1 & fleet_direct_emissions_state_breakdown$State == states_correspondence$COBRA_code[j])])
      #scenario$NO2[temp] <- baseline$NO2[temp]*yearly_change_NOx_emissions[m,j]
      #scenario$SO2[temp] <- baseline$SO2[temp]*yearly_change_SO2_emissions[m,j]
      ...
      if (fleet_electricity_consumption_source == "FLAME") {
        ...
        for (k in 1:dim(counties)[1]) {
          ...
          county_FIPS <- GIS_matching_matrix$FIPS[which(GIS_matching_matrix$COBRA_SOURCEINDX == counties$sourceindx[k])]
          county_pop_factor <- county_allocation_factor$county_allocation_factor[which(county_allocation_factor$FIPS == county_FIPS)]
          baseline$NO2[temp] <- fleet_emissions_state$NOx[which(fleet_emissions_state$Year == first_proj_yr & fleet_emissions_state$State == states_correspondence$COBRA_code[j])]*county_pop_factor/length(scenario$NO2[temp])
          baseline$SO2[temp] <- fleet_emissions_state$SO2[which(fleet_emissions_state$Year == first_proj_yr & fleet_emissions_state$State == states_correspondence$COBRA_code[j])]*county_pop_factor/length(scenario$NO2[temp])
          baseline$PM25[temp] <- (fleet_emissions_state$Total_PM25[which(fleet_emissions_state$Year == first_proj_yr & fleet_emissions_state$State == states_correspondence$COBRA_code[j])]+fleet_emissions_state$Brake_PM25[which(fleet_emissions_state$Year == first_proj_yr & fleet_emissions_state$State == states_correspondence$COBRA_code[j])]+fleet_emissions_state$Tire_PM25[which(fleet_emissions_state$Year == first_proj_yr & fleet_emissions_state$State == states_correspondence$COBRA_code[j])])*county_pop_factor/length(scenario$NO2[temp])
          # Calculation of the emissions from the fleet - county-based with data taken directly from MOVES (overwrites original data from COBRA)
          scenario$NO2[temp] <- fleet_emissions_state$NOx[which(fleet_emissions_state$Year == last_yr & fleet_emissions_state$State == states_correspondence$COBRA_code[j])]*county_pop_factor/length(scenario$NO2[temp])
          scenario$SO2[temp] <- fleet_emissions_state$SO2[which(fleet_emissions_state$Year == last_yr & fleet_emissions_state$State == states_correspondence$COBRA_code[j])]*county_pop_factor/length(scenario$NO2[temp])
          scenario$PM25[temp] <- (fleet_emissions_state$Total_PM25[which(fleet_emissions_state$Year == last_yr & fleet_emissions_state$State == states_correspondence$COBRA_code[j])]+fleet_emissions_state$Brake_PM25[which(fleet_emissions_state$Year == last_yr & fleet_emissions_state$State == states_correspondence$COBRA_code[j])]+fleet_emissions_state$Tire_PM25[which(fleet_emissions_state$Year == last_yr & fleet_emissions_state$State == states_correspondence$COBRA_code[j])])*county_pop_factor/length(scenario$NO2[temp])
          # Calculation of the emissions from electricity
          temp_sourceindx <- which(scenario$sourceindx == counties$sourceindx[k] & scenario$TIER1 == 1)
          temp_fips <- which(fleet_emissions_elec_county$FIPS == counties$FIPS[k] & fleet_emissions_elec_county$Year == last_yr)
          scenario$NO2[temp_sourceindx] <- baseline$NO2[temp_sourceindx] + (fleet_emissions_elec_county$NOx[temp_fips]-fleet_emissions_elec_county$NOx[which(fleet_emissions_elec_county$FIPS == counties$FIPS[k] & fleet_emissions_elec_county$Year == first_proj_yr+1)])/length(scenario$NO2[temp_sourceindx])
          scenario$SO2[temp_sourceindx] <- baseline$SO2[temp_sourceindx] + (fleet_emissions_elec_county$SO2[temp_fips]-fleet_emissions_elec_county$SO2[which(fleet_emissions_elec_county$FIPS == counties$FIPS[k] & fleet_emissions_elec_county$Year == first_proj_yr+1)])/length(scenario$SO2[temp_sourceindx])
          scenario$NH3[temp_sourceindx] <- baseline$NH3[temp_sourceindx] + (fleet_emissions_elec_county$NH3[temp_fips]-fleet_emissions_elec_county$NH3[which(fleet_emissions_elec_county$FIPS == counties$FIPS[k] & fleet_emissions_elec_county$Year == first_proj_yr+1)])/length(scenario$NH3[temp_sourceindx])
          scenario$PM25[temp_sourceindx] <- baseline$PM25[temp_sourceindx] + (fleet_emissions_elec_county$PM25[temp_fips]-fleet_emissions_elec_county$PM25[which(fleet_emissions_elec_county$FIPS == counties$FIPS[k] & fleet_emissions_elec_county$Year == first_proj_yr+1)])/length(scenario$PM25[temp_sourceindx])
          scenario$VOC[temp_sourceindx] <- baseline$VOC[temp_sourceindx] + (fleet_emissions_elec_county$VOC[temp_fips]-fleet_emissions_elec_county$VOC[which(fleet_emissions_elec_county$FIPS == counties$FIPS[k] & fleet_emissions_elec_county$Year == first_proj_yr+1)])/length(scenario$VOC[temp_sourceindx])
        }
      }
    }
    # Write the files in the input folder to be used by COBRA
  ...
  if (calculation_mode == "pollutants_breakdown" | calculation_mode == "all") {
    pollutants <- c("NOx", "SO2", "PM25Combustion", "PM25Tire", "PM25Brake")
    ...
    for (i in pollutants) {
      for (j in sectors) {
        ...
        for (k in 1:length(states_correspondence$COBRA_code)) {
          if (j == "OilIndustry") {
            temp_petroleum <- which(scenario$stid == states_correspondence$COBRA_code[k] & scenario$TIER1 == 6 & scenario$TIER2 == 1 & scenario$TIER3 == 99)
            temp_petroleum_storage <- which(scenario$stid == states_correspondence$COBRA_code[k] & scenario$TIER1 == 9 & scenario$TIER2 == 2 & (scenario$TIER3 == 2 | scenario$TIER3 == 4 | scenario$TIER3 == 6 | scenario$TIER3 == 8))
            temp_refining <- which(scenario$stid == states_correspondence$COBRA_code[k] & scenario$TIER1 == 6 & scenario$TIER2 == 2)
            temp_refined_storage <- which(scenario$stid == states_correspondence$COBRA_code[k] & scenario$TIER1 == 9 & scenario$TIER2 == 2 &(scenario$TIER3 == 1 | scenario$TIER3 == 3 | scenario$TIER3 == 5 | scenario$TIER3 == 7 | scenario$TIER3 == 9 | scenario$TIER3 == 10))
            temp_refined_storage_bis <- which(scenario$stid == states_correspondence$COBRA_code[k] & scenario$TIER1 == 9 & scenario$TIER2 == 3 & scenario$TIER3 != 5)
            temp_refined_distribution <- which(scenario$stid == states_correspondence$COBRA_code[k] & scenario$TIER1 == 9 & (scenario$TIER2 == 4 | scenario$TIER2 == 5 | scenario$TIER2 == 6))
            temp_ethanol <- which(scenario$stid == states_correspondence$COBRA_code[k] & scenario$TIER1 == 7 & scenario$TIER2 == 99 & scenario$TIER3 == 1)
            temp_ng <- which(scenario$stid == states_correspondence$COBRA_code[k] & scenario$TIER1 == 6 & scenario$TIER2 == 1 & scenario$TIER3 == 1)
            ...
            for (l in 1:dim(counties)[1]) {
              temp_sourceindx <- which(scenario$sourceindx == counties$sourceindx[l] & scenario$TIER1 == 1)
              temp_fips <- which(fleet_emissions_elec_county$FIPS == counties$FIPS[l] & fleet_emissions_elec_county$Year == last_yr)
              if (i == "NOx") {
                scenario$NO2[temp_sourceindx] <- baseline$NO2[temp_sourceindx] + (fleet_emissions_elec_county$NOx[temp_fips]-fleet_emissions_elec_county$NOx[which(fleet_emissions_elec_county$FIPS == counties$FIPS[l] & fleet_emissions_elec_county$Year == first_proj_yr+1)])/length(scenario$NO2[temp_sourceindx])
              } else if (i == "SO2") {
                scenario$SO2[temp_sourceindx] <- baseline$SO2[temp_sourceindx] + (fleet_emissions_elec_county$SO2[temp_fips]-fleet_emissions_elec_county$SO2[which(fleet_emissions_elec_county$FIPS == counties$FIPS[l] & fleet_emissions_elec_county$Year == first_proj_yr+1)])/length(scenario$SO2[temp_sourceindx])
              } else if (i == "PM25Combustion") {
                scenario$PM25[temp_sourceindx] <- baseline$PM25[temp_sourceindx] + (fleet_emissions_elec_county$PM25[temp_fips]-fleet_emissions_elec_county$PM25[which(fleet_emissions_elec_county$FIPS == counties$FIPS[l] & fleet_emissions_elec_county$Year == first_proj_yr+1)])/length(scenario$PM25[temp_sourceindx])
              }
            }
          }  else if (j == "Fleet") {
            # Emissions from the fleet
            ...
            colnames(counties) <- c("sourceindx", "FIPS")
            for (l in 1:dim(counties)[1]) {
              temp <- which(scenario$sourceindx == counties$sourceindx[l] & scenario$TIER1 == 11 & scenario$TIER3 == 2)
              county_FIPS <- GIS_matching_matrix$FIPS[which(GIS_matching_matrix$COBRA_SOURCEINDX == counties$sourceindx[l])]
              county_pop_factor <- county_allocation_factor$county_allocation_factor[which(county_allocation_factor$FIPS == county_FIPS)]
              if (i == "NOx") {
                baseline$NO2[temp] <- fleet_emissions_state$NOx[which(fleet_emissions_state$Year == first_proj_yr & fleet_emissions_state$State == states_correspondence$COBRA_code[k])]*county_pop_factor/length(scenario$NO2[temp])
                scenario$NO2[temp] <- fleet_emissions_state$NOx[which(fleet_emissions_state$Year == last_yr & fleet_emissions_state$State == states_correspondence$COBRA_code[k])]*county_pop_factor/length(scenario$NO2[temp])
              } else if (i == "SO2") {
                baseline$SO2[temp] <- fleet_emissions_state$SO2[which(fleet_emissions_state$Year == first_proj_yr & fleet_emissions_state$State == states_correspondence$COBRA_code[k])]*county_pop_factor/length(scenario$NO2[temp])
                scenario$SO2[temp] <- fleet_emissions_state$SO2[which(fleet_emissions_state$Year == last_yr & fleet_emissions_state$State == states_correspondence$COBRA_code[k])]*county_pop_factor/length(scenario$NO2[temp])
              } else if (i == "PM25Combustion") {
                baseline$PM25[temp] <- (fleet_emissions_state$Total_PM25[which(fleet_emissions_state$Year == first_proj_yr & fleet_emissions_state$State == states_correspondence$COBRA_code[k])]+fleet_emissions_state$Brake_PM25[which(fleet_emissions_state$Year == first_proj_yr & fleet_emissions_state$State == states_correspondence$COBRA_code[k])]+fleet_emissions_state$Tire_PM25[which(fleet_emissions_state$Year == first_proj_yr & fleet_emissions_state$State == states_correspondence$COBRA_code[k])])*county_pop_factor/length(scenario$NO2[temp])
                scenario$PM25[temp] <- (fleet_emissions_state$Total_PM25[which(fleet_emissions_state$Year == last_yr & fleet_emissions_state$State == states_correspondence$COBRA_code[k])]+fleet_emissions_state$Brake_PM25[which(fleet_emissions_state$Year == first_proj_yr & fleet_emissions_state$State == states_correspondence$COBRA_code[k])]+fleet_emissions_state$Tire_PM25[which(fleet_emissions_state$Year == first_proj_yr & fleet_emissions_state$State == states_correspondence$COBRA_code[k])])*county_pop_factor/length(scenario$NO2[temp])
              } else if (i == "PM25Tire") {
                baseline$PM25[temp] <- (fleet_emissions_state$Total_PM25[which(fleet_emissions_state$Year == first_proj_yr & fleet_emissions_state$State == states_correspondence$COBRA_code[k])]+fleet_emissions_state$Brake_PM25[which(fleet_emissions_state$Year == first_proj_yr & fleet_emissions_state$State == states_correspondence$COBRA_code[k])]+fleet_emissions_state$Tire_PM25[which(fleet_emissions_state$Year == first_proj_yr & fleet_emissions_state$State == states_correspondence$COBRA_code[k])])*county_pop_factor/length(scenario$NO2[temp])
                scenario$PM25[temp] <- (fleet_emissions_state$Total_PM25[which(fleet_emissions_state$Year == first_proj_yr & fleet_emissions_state$State == states_correspondence$COBRA_code[k])]+fleet_emissions_state$Brake_PM25[which(fleet_emissions_state$Year == first_proj_yr & fleet_emissions_state$State == states_correspondence$COBRA_code[k])]+fleet_emissions_state$Tire_PM25[which(fleet_emissions_state$Year == last_yr & fleet_emissions_state$State == states_correspondence$COBRA_code[k])])*county_pop_factor/length(scenario$NO2[temp])
  ...
  if (calculation_mode == "default" | calculation_mode == "all") {
    ...
    health_benefits_by_county <- select(cobra_output_file, c(Destination, colnames(cobra_output_file)[which(grepl(pattern = "X..", colnames(cobra_output_file)))], year)) %>%
      add_column("State" = NA) %>%
      add_column("County" = NA) %>%
      add_column("FIPS" = NA) %>%
      add_column("Total_health_benefits" = NA)
    health_benefits_by_county$Total_health_benefits <- rowSums(cobra_output_file[which(grepl(pattern = "X..", colnames(cobra_output_file)))])
    ...
    n <- 0
    for (i in (first_proj_yr+1):last_yr) {
      health_benefits <- add_row(health_benefits, Year = i, Total_health_benefits = sum(cobra_output_file[which(is.na(cobra_output_file$Destination) & cobra_output_file$year == i), which(grepl(pattern = "X..", colnames(cobra_output_file)))]))
      for (j in 1:length(states_correspondence$COBRA_code)) {
        n <- n+1
        health_benefits_by_state <- add_row(health_benefits_by_state, Year = i, State = states_correspondence$COBRA_code[j], Total_health_benefits = sum(cobra_output_file[which(cobra_output_file$state_id == states_correspondence$COBRA_code[j] & cobra_output_file$year == i), which(grepl(pattern = "X..", colnames(cobra_output_file)))]))
      }
    }
  ...
  if (calculation_mode == "aggregated" | calculation_mode == "all") {
    print("Calculating the health benefits by county")
    FIPS_counties <- get_input_f(input_name = 'COBRA_SOURCEINDX to FIPS crosswalk')
    health_benefits_by_county <- select(cobra_output_file, c(Destination, colnames(cobra_output_file)[which(grepl(pattern = "X..", colnames(cobra_output_file)))])) %>%
      add_column("State" = NA) %>%
      add_column("County" = NA) %>%
      add_column("FIPS" = NA) %>%
      add_column("Total_health_benefits" = NA)
  ...
  if (calculation_mode == "pollutants_breakdown" | calculation_mode == "all") {
    print("Calculating the health benefits by county")
    FIPS_counties <- get_input_f(input_name = 'COBRA_SOURCEINDX to FIPS crosswalk')
    health_benefits_by_county <- select(cobra_output_file, c(Destination, colnames(cobra_output_file)[which(grepl(pattern = "X..", colnames(cobra_output_file)))], Pollutant, Sector)) %>%
      add_column("State" = NA) %>%
      add_column("County" = NA) %>%
      add_column("FIPS" = NA) %>%
      add_column("Total_health_benefits" = NA)
