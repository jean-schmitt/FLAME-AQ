...
backcasting_procedure_f <- function(s_and_f_ghg=NA,
  ...
  fleet_emission_target_f_res <- do.call(fun_res_f,list(fun_name="fleet_emission_target_f"))
  fleet_emission_target <- fleet_emission_target_f_res[["fleet_emission_target"]]
  
  # Calculate the emission profile in default case --------------------------

  fleet_climate_change_metric_f_ref <- do.call(fun_res_f,list(fun_name="fleet_climate_change_metric_f"))
