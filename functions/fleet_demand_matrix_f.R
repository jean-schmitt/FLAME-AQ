...
fleet_demand_matrix_f<-function(results_path, first_yr=NA,last_yr=NA, fuel_matching_option = NA){
  ...
  fleet_demand_matrix[lca_process$Phase=="Fuel Production" & lca_process$Process=="E-fuel-Fuel distribution",] <- acast(data=subset(fleet_efuel_dt,Fuel=="Efuel" & Year%in%first_yr:last_yr),Fuel ~ Year , value.var='Value',fun.aggregate=sum, margins=FALSE)[,colnames(fleet_demand_matrix)]
