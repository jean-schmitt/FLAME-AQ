...
fleet_i_mat_cont_f <- function (mat_cont_adj=NA) {
  ...
  #Input files
  ori_mat_comp  <- get_input_f(input_name = 'component_material_comp_acsm')
  adj_mat_comp  <- get_input_f(input_name = 'component_material_comp_adj')
  material  <- get_input_f(input_name = 'model_matching_material')
  vh_techno  <- get_input_f(input_name = 'model_matching_technology')
  comp_dt  <- get_input_f(input_name = 'model_matching_component')
  #Function inputs
  fleet_i_comp_wgt_f_res <- do.call(fun_res_f,list(fun_name="fleet_i_comp_wgt_f"))
    ...
    for (techno in unique(vh_techno$Own)){
      ...
      for (subcomp in subcomponent){
        ...
        for (mat in unique(material$Own)) {
          ...
          mat_compo <- subset(fleet_subcompo_wgt,Size==size & Technology==techno & Subcomponent==subcomp)[,"Weight"]*mat_comp[row_tbc,mat]
          #Fill output file
