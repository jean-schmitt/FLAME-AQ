...
#' @export
fleet_vint_stock_initialization_f <- function(fleet, first_proj_yr = NA){
  ...
  #matrix_survived_pop represents the theoritical normalized survived population distribution based on the cumulative survivale rates
  matrix_survived_pop <- t(sapply(rownames(fleet$ldv_sales),function(x) do.call(survival_rate_f,list(year=i_year, size=strsplit(x,"_")[[1]][1], techno=strsplit(x,"_")[[1]][2], cumulative_rate="y"))/sum(do.call(survival_rate_f,list(year=i_year, size=strsplit(x,"_")[[1]][1], techno=strsplit(x,"_")[[1]][2], cumulative_rate="y")))))
  #The first year stock is based on the survive pop distirbution and the initial stock (minus new vehicles)
  matrix_vint_stock[,as.character(1:last_age_tbc)] <- vector_remaining_stock[rownames(matrix_vint_stock),] * matrix_survived_pop
  #Update fleet with first data
  fleet$vint_stock[[as.character(i_year)]] <- trunc(matrix_vint_stock)
