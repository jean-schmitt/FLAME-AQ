...
fleet_technology_market_share_proj_f <- function(market_share_source=NA,
  ...
  proj_sales_aeo <- left_join(proj_sales_aeo, correspondence_file, by = "Technology") %>%
    select(-c("ID", "Technology")) %>%
    relocate("Technology_Alt", .before = "Size")
  colnames(proj_sales_aeo)[1] <- "Technology"
  proj_sales_aeo <- aggregate(.~Technology + Size + Year + Data_type + Aeo_case + Aeo_year + Unit, data = proj_sales_aeo, FUN = sum)
  ...
  } else if (market_share_source=="hybridization"){
    #Input
  ...
  if (market_share_adj_techno=="bnef"){
    ...
    for (size in size_to_adjust){
      ...
      proj_values <- approx(x=c(market_share_adj_start_yr,2035), y=c(i_value,1), xout=seq(last_hist_yr,last_yr,1),yleft=i_value,yright=1, method = "linear")$y
