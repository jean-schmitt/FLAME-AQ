...
fleet_vkt_f <- function(first_yr = NA, last_yr = NA, vkt_mdl=NA, vkt_hybrid_factor=NA, vkt_growth_mdl=NA, vkt_growth_rate=NA, fuel_matching_option = NA, fleet_id = NA, use_res_env="y"){
  ...
  #Function outputs
  fleet_vint_stock_f_res <- do.call(fun_res_f,list(fun_name="fleet_vint_stock_f",res_env=use_res_env))
  ...
  } else if (vkt_growth_mdl == "Constant"){
    #No growth in VKMT
    vkt_growth_f<-function(y){
      return(1)
    }
  } else if (vkt_growth_mdl == "linear_def"){
  ...
  } else if (vkt_growth_mdl == "linear_high"){
    #Growth in VKMT following linear trends based on historical calculation
    vkt_growth_f<-function(y){
      growth_rate = 0.008
      ref_y = 2019
  ...
  } else if (vkt_growth_mdl == "linear_low"){
    #Growth in VKMT following linear trends based on historical calculation
    vkt_growth_f<-function(y){
      growth_rate = -0.008
      ref_y = 2019
  ...
  for (size in unique(fleet_vint_stock$Size)) {  
    for (techno in unique(vh_techno$Own)) {
      ...
      } else if (vkt_mdl=="constant"){
        vec_annual_kt <- matrix(20376,ncol = length(unique(stock$Age)),nrow = 1,dimnames = list("Annual KT per vehicle",unique(stock$Age)))
      } else if (vkt_mdl=="hybrid"){
        vec_annual_kt <- matrix((1-vkt_hybrid_factor)*20376+vkt_hybrid_factor*annual_mileage[order(annual_mileage$`Vehicle age`),size]*conv["km","1 mile"],ncol = length(unique(stock$Age)),nrow = 1,dimnames = list("Annual KT per vehicle",unique(stock$Age)))
      }
  ...
  results<-list(fleet_vint_vkt=fleet_vint_vkt,
                ...
                fleet_composition = fleet_vint_stock_f_res)
  return(results)
}
