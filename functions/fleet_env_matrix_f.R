...
#' @export
fleet_env_matrix_f <- function(first_yr=NA,last_yr=NA,ghg=NA, first_proj_yr = NA){
  ...
  lca_ef_elec <- subset(lca_ef_elec_f_res[["lca_ef_elec"]],Year%in%first_proj_yr:last_yr)
  #
  lca_ef_efuel_elec_f_res <- do.call(lca_ef_efuel_elec_f,list(ghg=ghg))
  lca_ef_efuel_elec <- subset(lca_ef_efuel_elec_f_res[["lca_ef_elec"]],Year%in%first_proj_yr:last_yr)
  #
  ...
  #Output
  fleet_env_matrix <- matrix(0,ncol = length(first_proj_yr:last_yr),nrow = nrow(lca_process),dimnames = list(paste(lca_process$Phase,lca_process$Process),first_proj_yr:last_yr))
  ...
  #Fill environmental matrix with emissions factors for electricity production for EV electricity use
  ...
  #Fill environmental matrix with emissions factors for electricity production associated with efuel
  fleet_env_matrix[which(lca_process$Phase=="Fuel Production" & lca_process$Process%in%c("E-fuel-H2 production","E-fuel-Syngas production","E-fuel-CO2 capture")),] <- matrix(lca_ef_efuel_elec$Value[order(lca_ef_efuel_elec$Year)],nrow=3,ncol=ncol(fleet_env_matrix),byrow = TRUE) 
