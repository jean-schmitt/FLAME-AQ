fleet_composition_scenario_f <- function(fleet, fleet_initial_year = NA, fleet_final_year = NA, first_proj_yr = NA, fleet_id = NA) {
  ...
  for (i in 2:length(colnames(scenario_entire_us))) {
    scenario_entire_us[,i] <- tolower(scenario_entire_us[,i])
  }
  ...
  for (iterations in 1:length(state_groups)) {
    ...
    # Replace here with a lapply to speed up the calculation 
    #index <- fleet_vint_stock_scenario_temp[which(fleet_vint_stock_scenario_temp$Year >= (fleet_initial_year-1)),]
    #output <- future_lapply(index, function(i) {
    #  stock_old <- fleet_vint_stock_scenario_temp$Value[which(fleet_vint_stock_scenario_temp$Year ==  fleet_vint_stock_scenario_temp$Year[i] & fleet_vint_stock_scenario_temp$Size == fleet_vint_stock_scenario_temp$Size[i] & fleet_vint_stock_scenario_temp$Technology == fleet_vint_stock_scenario_temp$Technology[i] & fleet_vint_stock_scenario_temp$Age == fleet_vint_stock_scenario_temp$Age[i]-1)]
    #  survival_rate <- fleet_vint_survival_rate_scenario$Value[which(fleet_vint_survival_rate_scenario$Year == fleet_vint_stock_scenario_temp$Year[i] & fleet_vint_survival_rate_scenario$Size == fleet_vint_stock_scenario_temp$Size[i] & fleet_vint_survival_rate_scenario$Technology == fleet_vint_stock_scenario_temp$Technology[i] & fleet_vint_survival_rate_scenario$Age == fleet_vint_stock_scenario_temp$Age[i])]
