...
#' @export
vehicle_lca_demand_matrix_f<-function(techno,size,model_year,lca_veh_lifetime_mdl=NA,first_yr=NA,last_yr=NA,efuel_dep_mdl=NA,efuel_dep_rate=NA,efuel_param_scen=NA){
  ...
  mat_efuel_dep <- matrix(0,nrow=1,ncol=length(first_yr:last_yr),dimnames=list("Efuel ratio",first_yr:last_yr))
  #Last 
  last_hist_yr <- 2021
  if (efuel_dep_mdl=="y"){
  ...
  mat_efuel_feedstock_factor <- matrix(subset(efuel_parameters)$Value,nrow=length(feedstock_list),ncol=ncol(mat_efuel_dep),dimnames=list(feedstock_list,colnames(mat_efuel_dep)))
  #Functions' Outputs
  ...
  for (fuel in fuel_l){
    ...
    if(fuel!="Gasoline"){
      ...
      #Fill Gasoline
      vehicle_demand_matrix[lca_process$Phase=="Fuel Production" & lca_process$Process==fuel,as.character(vehicle_op_year_list)] <- fc/100*uf*annual_vkt_dt$`Annual VKT`*(1-mat_efuel_dep[,as.character(vehicle_op_year_list)])
      vehicle_demand_matrix[lca_process$Phase=="Fuel Use" & lca_process$Process==fuel,as.character(vehicle_op_year_list)] <- fc/100*uf*annual_vkt_dt$`Annual VKT`*(1-mat_efuel_dep[,as.character(vehicle_op_year_list)])
      #Fill e-fuel
      mat_efuel <- fc/100*uf*annual_vkt_dt$`Annual VKT`*mat_efuel_dep[,as.character(vehicle_op_year_list),drop = FALSE]*efuel_adj_factor
      ##mat_efuel_feedstock is the matrix of feedstock needed to fulfill the volume of efuel
      mat_efuel_feedstock <- mat_efuel_feedstock_factor[,as.character(vehicle_op_year_list)] * matrix(mat_efuel[1,],nrow=length(feedstock_list),ncol=ncol(mat_efuel),byrow = TRUE)
      #Fill vehicle demand matrix
      vehicle_demand_matrix[lca_process$Phase=="Fuel Production" & lca_process$Process=="E-fuel-Fuel synthesis",as.character(vehicle_op_year_list)] <- mat_efuel
      vehicle_demand_matrix[lca_process$Phase=="Fuel Production" & lca_process$Process=="E-fuel-Fuel distribution",as.character(vehicle_op_year_list)] <- mat_efuel
      vehicle_demand_matrix[lca_process$Phase=="Fuel Production" & lca_process$Process=="E-fuel-H2 production",as.character(vehicle_op_year_list)] <- mat_efuel_feedstock["Electricity for H2",]
      vehicle_demand_matrix[lca_process$Phase=="Fuel Production" & lca_process$Process=="E-fuel-Syngas production",as.character(vehicle_op_year_list)] <- mat_efuel_feedstock["Electricity for syngas",]
      vehicle_demand_matrix[lca_process$Phase=="Fuel Production" & lca_process$Process=="E-fuel-CO2 capture",as.character(vehicle_op_year_list)] <- mat_efuel_feedstock["Electricity for CO2",]
    }
  }
