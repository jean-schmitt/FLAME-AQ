...
fleet_mfa_f <- function(rec_scen=NA,first_yr = NA,last_yr = NA, first_proj_yr = NA) {
  ...
  vh_techno <- filter(vh_techno, vh_techno$Own%in%unique(fleet_vint_stock$Technology))
  ...
  mat_emb <- matrix(0,ncol = length(first_proj_yr:last_yr),nrow = length(material_dt$Own),dimnames = list(material_dt$Own,first_proj_yr:last_yr))
  ...
  mat_scrap <- matrix(0,ncol = length(first_proj_yr:last_yr),nrow = length(material_dt$Own),dimnames = list(material_dt$Own,first_proj_yr:last_yr))
  ...
  mat_rec <- matrix(0, ncol = length(first_proj_yr:last_yr), nrow = length(rec_mat_l),dimnames = list(rec_mat_l,first_proj_yr:last_yr))
  ...
  for (size in size_l) {
    ...
    for (techno in unique(vh_techno$Own)) {
      ...
      new_sales <- subset(fleet_vint_stock, Size==size & Technology == techno & Year%in%c(first_proj_yr:last_yr) & Age==0)
      #if (dim(new_sales)[1] == 0) {
      #  for (years in first_proj_yr:last_yr) {
      ...
      tmp_matrix_emb <- matrix_mt_comp[rownames(mat_emb),as.character(first_proj_yr:last_yr)] %*% matrix_new[as.character(first_proj_yr:last_yr),as.character(first_proj_yr:last_yr)]
      mat_emb <- mat_emb + tmp_matrix_emb[rownames(mat_emb),colnames(mat_emb)]
      #Extract the stock of scrapped vehicles
      scrap <- subset(fleet_vint_scrap, Size==size & Technology == techno & Year%in%c(first_proj_yr:last_yr))
      if (dim(scrap)[1] == 0) {
        for (years in first_proj_yr:last_yr) {
          for (model_years in first_proj_yr:years) {
      ...
      scrap <- scrap %>% subset(select=c(Year,Model_year,Value)) %>% aggregate(Value ~ Year + Model_year,data=.,FUN = sum)
      #Loop for year
  ...
  for (y in first_proj_yr:last_yr) {
